...
<body>
<center><h1><a href="./">STL</a>: SHARED File Manager (SEARCH)</h1></center>

<div id="wrapper">
<div class="path">
<br/>
<br/>
Notes about using the <b>search</b> feature:
<ul>
<li>Search is very simple (<b>basic regex</b> in the backend) matching keywords found in file names.</li>
<li>If the file name is not informative, then you may need to navigate to the folder to find what you're looking for.</li>
<li>Most files uploaded to this repository contain the first author's surname and the title of the paper/book/document.</li>
</ul>
<br/>
<br/>
<form action="" method="get"><input name="q" style="width: 300px;" type="text"> <input type="submit" ></form>
<br/>
<?php
...
foreach(new RecursiveIteratorIterator($it) as $file)
{
	$name = basename($file);
	$pos = strpos($name, $q);
    if ($pos >1 && in_array(strtolower(array_pop(explode('.', $file))), $display))
//    	echo "<a href='$dir/$file'>$file</a>"; 
//	    echo "<br>"; 
...
/**
$folder = ".";
$pattern = "*index*";  //the word you are looking for

// $pattern = '*'.$q.'*';
$files = glob($folder. '/' . $pattern, GLOB_BRACE);
foreach($files as $filename) {
echo "found something";
echo "<a href='$folder/$filename'>$filename</a>" . "<br/> \n";
}

   print_r(glob("*kinship*"));
**/
...
// print_r(rsearch(".","/^.*\.(pdf|txt|html)$/"));
// print_r(rsearch("./","/^.*index.*\.*$/i"));
...
function rsearch($folder, $pattern) {
  ...
  foreach($files as $file) {
    $fileList[] = $file[0]. "<br />";
    echo "";
    echo "<li><a href='$file[0]' target=\"_blank\">$file[0]</a></li>" . "<br/> \n";
    echo "";
  }
