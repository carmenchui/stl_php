...
$exclude_items = array('indextest.php','index_old.php','index.php','sitemap.php','resources','Miscellaneous','anywhereinfiles.php','TestFolder', 'hidden');
...
if (is_array($objects)){// && fm_is_exclude_items($current_path)) {
    foreach ($objects as $file) {
        ...
        if ($file == 'index.php' || $file == 'indextest.php'|| $file == 'sitemap.php' || $file == 'index_old.php'|| $file == 'anywhereinfiles.php' || $file == 'index (2020-08-31, 12:58).php'){
        	break;
        }
        $new_path = $path . '/' . $file;
...
<head>
<meta charset="utf-8">
<title>STL: SHARED File Manager (Recent Uploads)</title>
<style>
...
<body>
<center><h1><a href="./">STL</a>: SHARED File Manager (Recent Uploads)</h1></center>
<div id="wrapper">
This is a listing of all recently uploaded files on the SHARED folder.<br/><br/>
...
function latestlimit($searchDir, $number, array $files = array()) {
    $search = opendir($searchDir);
   ...
   $num = (int) $number; 
   // echo '<br />'.$num;
    $files = array_slice($files, 0, $num, true);
    return $files;
}
...
//     $replacements = array(' ','!', '*', "'", "(", ")", ";", ":", "@", "&", "=", "+", "$", ",", "/", "?", "%", "#", "[", "]");
    $replacements = array('/',',',' ','-');
    $entities = array('%2F','%2C','+','-');


    return str_replace($replacements, $entities, urlencode($string));
}

function printlisting($latestfiles){
	echo '<ol>';
    foreach ($latestfiles as $f){
    /***
    echo 'f is '.$f;
    echo '<Br />';
        echo 'base name is '.basename($f);
    echo '<Br />';
        echo 'file name is '.pathinfo($f);
    echo '<Br />';
        echo 'dir name is '.dirname($f);
        ***/
//             	echo '<li><a href="'.$f.'">'.$f.'</a></li>'; // original
//     	http://stl.mie.utoronto.ca/internal/SHARED/index.php?p=Books%2FArp%2C+Smith%2C+Spear+-+Building+Ontologies+with+Basic+Formal+Ontology&view=Building+Ontologies+with+Basic+Formal+Ontology.pdf
// Books/Arp, Smith, Spear - Building Ontologies with Basic Formal Ontology/Building Ontologies with Basic Formal Ontology.pdf
$dirname = dirname($f);
$f2 = substr($dirname,2);
// $f = str_replace(' ','%20');
$f2 = myUrlEncode($f2);
$f3 = myUrlEncode(basename($f));    	
echo '<li><a href="http://stl.mie.utoronto.ca/internal/SHARED/index.php?p='.$f2.'&view='.$f3.'">'.$f.'</a></li>';
...
function fm_show_listings()
{
	echo '<br /><b>Most Recent Uploads in <u>Books</u>:</b><br /><br />';
	$latestfiles = latestlimit('./Books','15');
	printlisting($latestfiles);
	
	echo '<br /><b>Most Recent Uploads in <u>Conference & Workshop Proceedings</u>:</b><br /><br />';
	$latestfiles = latestlimit('./Conference & Workshop Proceedings','15');
	printlisting($latestfiles);	

	echo '<br /><b>Most Recent Uploads in <u>IEEE Standards</u>:</b><br /><br />';
	$latestfiles = latestlimit('./IEEE Standards','15');
	printlisting($latestfiles);	

	echo '<br /><b>Most Recent Uploads in <u>ISO Standards</u>:</b><br /><br />';
	$latestfiles = latestlimit('./ISO Standards','15');
	printlisting($latestfiles);	
	
	echo '<br /><b>Most Recent Uploads in <u>Journals</u>:</b><br /><br />';
	$latestfiles = latestlimit('./Journals','15');
	printlisting($latestfiles);	
	
	echo '<br /><b>Most Recent Uploads in <u>Papers By Topic</u>:</b><br /><br />';
	$latestfiles = latestlimit('./Papers By Topic','15');
	printlisting($latestfiles);	
	
	echo '<br /><b>Most Recent Uploads in <u>Webinars</u>:</b><br /><br />';
	$latestfiles = latestlimit('./Webinars','15');
	printlisting($latestfiles);	
