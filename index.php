...
$root_path =  '.';//$_SERVER['DOCUMENT_ROOT'].'SHARED';

// Root url for links in file manager.Relative to $http_host. Variants: '', 'path/to/subfolder'
// Will not working if $root_path will be outside of server document root
$root_url = 'internal/SHARED';
...
$datetime_format = 'Y-m-d h:i A';//Y-m-d h:i:s A

// Files and folders to excluded from listing
// e.g. array('myfile.html', 'personal-folder', '*.php', ...)
$exclude_items = array('indextest.php','index_old.php','index.php','sitemap.php','resources','Miscellaneous','anywhereinfiles.php','TestFolder', 'hidden','css','files','img','bower.json','composer.json','elfinder.html','elfinder.legacy.html','.quarantine','.tmb');
...
if (is_array($objects)){// && fm_is_exclude_items($current_path)) {
    foreach ($objects as $file) {
        ...
        if ($file == 'index.php' || $file == 'indextest.php'|| $file == 'sitemap.php' || $file == 'index_old.php'|| $file == 'anywhereinfiles.php' || $file == 'index (2020-08-31, 12:58).php' || $file == 'css'|| $file == 'files'|| $file == 'img'|| $file == 'bower.json'|| $file == 'composer.json'|| $file == 'elfinder.html'|| $file == 'elfinder.legacy.html'){
        	break;
        }
        $new_path = $path . '/' . $file;
...
if (isset($_GET['upload'])) {
    ...
    fm_show_nav_path_upload(FM_PATH); // current path
    ?>
<div class="path">
<b><u>Uploading Files</u></b><br /><br />
<ul>
<li><b>Destination Folder</b>: <a href="<?php echo fm_enc(fm_convert_win(FM_ROOT_PATH . '/' . FM_PATH)) ?>"><?php echo fm_enc(fm_convert_win(FM_ROOT_PATH . '/' . FM_PATH)) ?></a></li>
<li><b>File Format</b> for files: <b><font color="#4E12BB">Author Name(s)</font> - <font color="purple">Paper Name</font>.pdf</b> (use "et al" if multiple authors); e.g.,
<ul>
<li><a href="?p=Papers+By+Topic%2FProcess+Specification+Language+%28PSL%29&view=Gruninger+-+Ontology+of+the+Process+Specification+Language.pdf">Gruninger - Ontology of the Process Specification Language.pdf</a></li>
<li><a href="?p=Papers+By+Topic%2FProcess+Specification+Language+%28PSL%29&view=Schlenoff%2C+Gruninger%2C+Tissot+-+The+Process+Specification+Language+%28PSL%29.pdf">Schlenoff, Gruninger, Tissot - The Process Specification Language (PSL).pdf</a></li>
<li><a href="?p=Papers+By+Topic%2FUnified+Foundational+Ontology+%28UFO%29&view=Guizzardi+et+al+-+Grounding+Software+Domain+Ontologies+in+the+Unified+Foundational+Ontology+%28UFO%29.pdf">Guizzardi et al - Grounding Software Domain Ontologies in the Unified Foundational Ontology (UFO).pdf</a></li>
</ul>
</li>
</ul>
...
$filepath=fm_enc($file_url);//basename($file);

// also works
// echo "<iframe src=\"$filepath\" width=\"100%\" style=\"height:100%\"></iframe>";
// echo "<embed src=\"$filepath\" type=\"application/pdf\" width=\"100%\" style=\"height:150%\">";

echo "<object width=\"100%\" height=\"700px\" data=\"$filepath\"></object>";
...
<th style="width:12%">Modified<br />(YYYY-mm-dd)</th>
<!--<?php if (!FM_IS_WIN): ?><th style="width:6%">Permissions</th>
<th style="width:10%">Owner</th><?php endif; ?>-->
<th style="width:13%">Action</th></tr>
<?php
...
<a title="Rename" href="#" onclick="rename('<?php echo fm_enc(FM_PATH) ?>', '<?php echo fm_enc($f) ?>');return false;">Rename <i class="icon-rename"></i></a>
<a title="Copy to..." href="?p=&amp;copy=<?php echo urlencode(trim(FM_PATH . '/' . $f, '/')) ?>">Copy <i class="icon-copy"></i></a> -->
<a title="Direct link" href="<?php echo fm_enc(FM_ROOT_URL . (FM_PATH != '' ? '/' . FM_PATH : '') . '/' . $f . '/') ?>" target="_blank">Link <i class="icon-chain"></i></a>
</td></tr>
    <?php
    flush();
}
...
<tr>
<td><label><input type="checkbox" name="file[]" value="<?php echo fm_enc($f) ?>"></label></td>
<td><div class="filename"><a href="<?php echo fm_enc($filelink) ?>" title="<?php echo fm_enc(fm_convert_win($f)) ?>"><i class="<?php echo $img ?>"></i> <?php echo fm_enc(fm_convert_win($f)) ?></a><?php echo ($is_link ? ' &rarr; <i>' . fm_enc(readlink($path . '/' . $f)) . '</i>' : '') ?></div></td>
...
<a title="Rename" href="#" onclick="rename('<?php echo fm_enc(FM_PATH) ?>', '<?php echo fm_enc($f) ?>');return false;">Rename <i class="icon-rename"></i></a>
<a title="Copy to..." href="?p=<?php echo urlencode(FM_PATH) ?>&amp;copy=<?php echo urlencode(trim(FM_PATH . '/' . $f, '/')) ?>">Copy <i class="icon-copy"></i></a> -->
<a title="Direct link" href="<?php echo fm_enc(FM_ROOT_URL . (FM_PATH != '' ? '/' . FM_PATH : '') . '/' . $f) ?>" target="_blank">Link <i class="icon-chain"></i></a>
<a title="Download" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;dl=<?php echo urlencode($f) ?>">Download <i class="icon-download"></i></a>
</td></tr>
    <?php
    flush();
}
...
<input type="submit" name="zip" value="Create Zip of Selected Files " onclick="return confirm('Create archive?')"><i class="icon-download"></i>
...
<a title="Search files"  href="latest.php"><font color="red">Most Recent Uploads</font> <i class="icon-document"></i></a>
<a title="Search files"  href="search.php">Search Files <i class="icon-folder"></i></a>
<a title="Upload files" href="?p=UPLOADS&amp;upload">Upload Files to /UPLOADS<i class="icon-upload"></i></a>
<!--a title="New folder" href="#" onclick="newfolder('UPLOADS');return false;">Create Folder in /UPLOADS<i class="icon-folder_add"></i></a-->
 ...
 * @param string $path
 */
function fm_show_nav_path_upload($path)
{
    ?>
<div class="path">
<div class="float-right">
<a title="Search files"  href="search.php">Search Files <i class="icon-folder"></i></a>
<!-- <a title="Upload files" href="?upload">Upload Files <i class="icon-upload"></i></a> -->
<!-- <a title="New folder" href="#" onclick="newfolder('<?php echo fm_enc(FM_PATH) ?>');return false;">Create Folder <i class="icon-folder_add"></i></a> -->
<?php if (FM_USE_AUTH): ?><a title="Logout" href="?logout=1"><i class="icon-logout"></i></a><?php endif; ?>
</div>
        <?php
        $path = fm_clean_path($path);
...
<div id="wrapper">
<!-- <p class="center"><b>Disclaimer:</b> copying, moving, and deleting files in this web interface have been <b>disabled</b> for security reasons.</p> -->
<br />
This is a <b>read-only</b> interface to view shared files hosted internally on the Semantic Technologies Lab (STL) server.
<br /><br />
If you would like to upload files, please do so in the <a href="?p=UPLOADS"><b>/UPLOADS</b></a> folder.
<ul>
...
<b>Finder Interface</b>: A Mac-like Finder interface for /SHARED: <a href="http://stl.mie.utoronto.ca/internal/SHARED/finder.html"><font color='blue'>http://stl.mie.utoronto.ca/internal/SHARED/finder.html</font></a>
<br />(the search is better with this interface, but direct-linking from this interface results in hashed URLs).<br /><br />
</p>
...
function latest($searchDir, array $files = array()) {
    ...
    while($item = readdir($search)) {
    	$ext = pathinfo($item, PATHINFO_EXTENSION); //get file extension
        if ($item == '.' || $item == '..' || $item == "index.php" || $item == "index_latest.php" || $ext == 'php') { continue; }
    ...
    krsort($files);
    $files = array_slice($files, 0, 15, true);
    //
    	   //  echo '<ol>';
//     foreach ($files as $f){
//     	echo '<li>'.$f.'</li>';
...
/** removed september 01, 2020 
	echo '<br /><b>Most Recent 15 Uploads:</b><br /><br />';
	$latestfiles = latest('.');
	
	echo '<ol>';
    foreach ($latestfiles as $f){// echo $f;
...
**/
//     echo '<hl>';
// 	echo '<b>Latest Uploads:</b><br /><br />';
//     echo '<ol>';
//     var_dump(top5mods('.'));
//     $latest_array = top5mods('.');
//     foreach ($latest_array as $f){
//     	echo '<li>'.$f.'</li>';
//     };
//     echo '</ol>';	
    ?>
...
<p class="center">Full Listing of Files: <a href="sitemap.php">Sitemap</a> | Search Files: <a href="search.php">Search</a> | List of Recently Uploaded Files: <a href="latest.php">Latest Files</a></p>
<p class="center"><small>System Time: <?php echo date("Y-m-d h:i:s A");?> (America/Toronto)</small></p>
</div>
<script>
function newfolder(p){var n=prompt('New folder name in UPLOADS','folder');if(n!==null&&n!==''){window.location.search='p='+encodeURIComponent(p)+'&new='+encodeURIComponent(n);}}
